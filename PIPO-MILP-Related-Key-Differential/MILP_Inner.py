...
fix_diff  = [0x0000000000000000]   # for same plain same cipher --> make fix true
fix_pos = [0]
...
def PrintInner(SolveList):
    ...
    for i in range(0,len(SolveList)):
        ...
        for k in range(0,8):
            buf = buf + "8 u" + str(SolveList[i][0]) + "_" + str(8*k + SolveList[i][1])
            if k != 7:
                buf = buf + " + "
        for k in range(0,8):
        ...
        for k in range(0,8):
            ...
            if k != 7:
                buf = buf + " + "
        for k in range(0,8):
            buf = buf + " - u" + str(SolveList[i][0]) + "_" + str(8*k + SolveList[i][1])
        buf = buf + " >= 0\n"
        opInner.write(buf)
        ...
        for k in range(0,int(len(convpbl)/convpbl_entry_size)):
            for l in range(0,convpbl_entry_size):
                if convpbl[convpbl_entry_size*k+l] > 0:
                    if l <=(s_box_size-1):
                        buf = buf + " + " + str(convpbl[convpbl_entry_size*k+l]) + " u" + str(SolveList[i][0]) + "_" + str((s_box_size*(s_box_size-1-l)) + SolveList[i][1])
...
bl = [[1, 5], [3, 5], [5, 5], [7, 5], [9, 5], [11, 5],[13,5]]
#bl = [[0, 7], [2, 7], [4, 7], [6, 7], [8, 7], [10, 7], [12, 7]]  # another characteristics
#bl = [[0, 5], [2, 5], [4, 5], [6, 5], [8, 5], [10, 5],[12,5]]  # For same input and same output
#key_active = [24, 77, 101]
BanList.append(bl)
BanListlen = len(bl)
PrintInner(bl)
while True:
